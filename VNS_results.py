...
if Me == 0:
    ...
    df_LM = pd.DataFrame(columns={'Gflops': LM_flops, 'Execution time (s)': LM_times,'Location': LM_ng}, index = ['LM'])
    df_VNS = pd.DataFrame(columns={'Gflops': VNS_flops, 'Execution time (s)': VNS_times,'Location': VNS_ng}, index = ['VNS'])
