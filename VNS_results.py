...
if Me == 0:
    ...
    df_LM = pd.DataFrame({'Gflops': LM_flops, 'Execution time (s)': LM_times,'Location': LM_ng}, index = initial)
    df_VNS = pd.DataFrame({'Gflops': VNS_flops, 'Execution time (s)': VNS_times,'Location': VNS_ng}, index = initial)
